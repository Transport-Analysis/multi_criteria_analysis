...
with st.expander('Import data from previously saved attempts (if applicable)', expanded=False):
    ...
    if uploaded_project is not None:
        ...
        project_description = pd.DataFrame(project_description)
    ...
    if not project_description.empty:  
        ...
        for _, row in project_description.iterrows():  
            ...
            if row.hasnans:             
                ...
                answers.append(','.join([x for x in st.multiselect(label=project_description.iloc[4,2], options=options, default=default)]))  
...
#### Step 4. Criteria ####
st.subheader("4. Criteria and 5. Criteria Ranking and Option Scoring")
...
with st.expander("Results", expanded=False):
    try:
        ...
        if len(UserInputs.columns) > 2 and len(UserInputs) > 1:
            ...
            st.write('Overall: %s' % OverallRank.index[OverallRank['Rank']==1][0])
