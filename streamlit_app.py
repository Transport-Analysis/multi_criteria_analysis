...
imported_project_descriptions = utils.import_project_descriptions().ProjectDescription
...
with st.expander('Import data from previously saved attempts (if applicable)', expanded=False):
    ...
    uploaded_project = st.file_uploader(
        'Upload Saved Excel Project (Files downloaded from this website only)',type='xlsx'
    )
...
with st.expander("Project Description", expanded=False):
    ...
    input_responses = [] 
    categories_used = []   
    options = [p.Options for p in imported_project_descriptions if p.Options is not None][0]
    ...
    if not project_description.empty:
        ...
        for proj_desc in imported_project_descriptions:
...
with st.expander("Define Options", expanded=False):
    ...
    if not option_description.empty:
        ...
        for _, row in option_description.iterrows():
            ...
            if row.Type == "User defined option":
                ...
                with col1:
                    option_name = st.text_input('User defined option', row.Option, key=row.Option)
                with col2:
                    option_desc = st.text_input(
                        'User defined option comment', row.OptionDescription, key=row.OptionDescription
                    )
                ...
                if row.Option == "Add new option":
                    ...
                    raise NotImplementedError('Only user defined and predefined options to be included')
    ...
    i = len([i for i in existing_options if existing_options[2] == 'User defined option']) + 1
    while True:
        col1, col2 = st.columns(2)
        with col1:
            new_option = st.text_input(f'What is option {i}?', 'Add new option', key=f'new_option{i}')
        with col2:
            new_option_comment = st.text_area(
                'Option description', 'Add new option comment', key=f'new_option_comment{i}')
        new_option_type = 'User defined option'
        if 'Add new option' in (new_option, new_option_comment):
            break
        else:
            new_options.append([new_option, new_option_comment, new_option_type])
            i+= 1
    ...
    uploaded_nof_solutions = [n[0] for n in existing_options if existing_options[2] == 'Predefined option']
    ...
    for nof_sol in nof_solutions:
        ...
        if nof_sol.Solution in uploaded_nof_solutions:
            with col1:
                checked = st.checkbox('', key=f'NOFsolutions{nof_i}', value=True)
            if checked:
                option_comment = [n[1] for n in existing_options if existing_options[0] == nof_sol.Solution][0]   
                with col3:
                    comment = st.text_input(
                        f'Additional comment on the solution: {nof_sol.Solution}', 
                        option_comment, 
                        key=f'NOFsolutions_cm{nof_sol.Solution}', 
                        help=option_comment
                    )       
        else:
            with col1:
                checked = st.checkbox('', key=f'NOFsolutions{nof_i}')
                if checked:
                    with col3:
                        option_comment = st.text_input(
                            f'Additional comment on the solution: {nof_sol.Solution}',
                            "Add comment", key=f'NOFsolutions_cm{nof_sol.Solution}', help=nof_sol.Comment
                        )
                    new_options.append([nof_sol.Solution, option_comment, 'Predefined option'])
        with col2:
            st.markdown(nof_sol.Solution)
    
    all_options = existing_options + new_options
    output_option_description = pd.DataFrame(all_options, columns=['Option', 'OptionDescription', 'Type'])
...
with st.expander("Criteria", expanded=False):
    ...
    criteria_df = pd.DataFrame(
        [[i.Category, i.Criterion, i.Indicator, i.Measure, i.NOS_mandatory] for i in criteria_list], 
        columns=['Category', 'Criterion', 'Indicator', 'Measure', 'NOS Mandatory']
    ...
    input_criteria_df = new_criteria_df[new_criteria_df.Criterion.apply(lambda x: x in user_inputs.index)]
    
    nos_flag = st.checkbox(''' Include all NOS Option's mandatory criteria''')
    nos_defaults = new_criteria_df.loc[new_criteria_df['NOS Mandatory'] == True].index if nos_flag else ''
    try:
        if user_inputs.empty:
            selected_rows = st.multiselect('Select rows:', new_criteria_df.index, default=[x for x in nos_defaults])
        else:
            selected_rows = st.multiselect(
                'Select rows:', new_criteria_df.index, 
                default=[x for x in list(set(nos_defaults) | set(input_criteria_df.index))]
            )
        selected_criteria = new_criteria_df.loc[selected_rows].sort_index()[['Category', 'Criterion']]
        ...
        while True:
            col1, col2 = st.columns(2)
            with col1:
                new_criteria = st.text_input(f'What is criteria {i}?', 'Add new criteria', key=f'new_criteria{i}')
            with col2:
                new_criteria_category = st.text_area(
                    'Criteria Category', 'Add new criteria category', key=f'new_criteria_comment{i}'
                )
        ...
        if len(new_custom_criteria) > 0:
            new_custom_criteria_df = pd.DataFrame(new_custom_criteria, columns=['Category', 'Criterion'])
        
        all_criteria_used_df = pd.concat([selected_criteria, new_custom_criteria_df])
        all_criteria_used_df = all_criteria_used_df[all_criteria_used_df['Criterion'] != 'Add new criteria'].copy(
...
with st.expander("Criteria Ranking & Scoring", expanded=False):
    ...
    for i, row in all_criteria_used_df.iterrows():
        ...
        label = 'Criteria Rank'
        index = available_ranks.index(row.Ranks) if row.Ranks in available_ranks else 0
        ...
        for _, option in output_option_description.iterrows():
            ...
            if option.Option in user_inputs.columns:
                key_value = user_inputs[user_inputs['Criterion'] == row.Criterion][option.Option]
            else:
                key_value = 3
            
            result = cols[i-1].select_slider(f'Score - option: {option.Option}', range(1, 6), key=key, value=key_value)
            updated_user_inputs.append([row.Criterion, option.Option, result])
            i+=1
    
        available_ranks = [x for x in available_ranks if x not in list(all_criteria_used_df.Ranks.unique())]
        
    updated_user_inputs = pd.DataFrame(updated_user_inputs, columns=['Criterion', 'Option', 'Value'])
...
with st.expander("Results", expanded=False):
    ...
    try:
        ...
        if not final_user_inputs.empty:
            ...
            plot = sns.barplot(x='Criterion', y='Value', hue='Option', data=updated_user_inputs, ax=ax1)
        ...
        rank_sums = [len(all_criteria_used_df) - x + 1 for x in all_criteria_used_df.Ranks]
        ...
        if len(final_user_inputs.columns) > 2 and len(final_user_inputs) > 1:
            ...
            overall_score_df = overall_score_df.transpose().sort_values(by='Score', ascending=False)
            overall_score_df = overall_score_df.style.format(subset=['Score'], formatter="{:.2}")
            overall_score_df
            ...
            overall_rank_df = overall_rank_df.transpose().sort_values(by='Rank')
            overall_rank_df
        
            #### Best Option ####
            st.subheader('Best Option')
            checked = st.checkbox('Exclude Base Case?')
            st.write('Overall: %s' % overall_rank_df.index[overall_rank_df['Rank'] == 1][0])     
                   
            scores_by_criteria = selected_criteria.copy().reset_index(drop=True)
            scores_df = pd.DataFrame(scores)
            scores_df.columns = ['Base Case'] + options
            scores_by_criteria_df = scores_df.join(scores_by_criteria).drop(columns=['Criterion'])
            if checked:
                scores_by_criteria_df = scores_by_criteria_df.drop(columns=['Base Case'])
            scores_by_criteria_df = pd.melt(scores_by_criteria_df, id_vars='Category').rename(columns={'variable': 'Option'})
            scores_by_criteria_df = scores_by_criteria_df.groupby(['Category', 'Option']).agg({'value': 'sum'}).reset_index().sort_values(by=['Category', 'value'], ascending=False)
            max_scores_by_criteria_df = scores_by_criteria_df.groupby(['Category']).agg({'value': 'max'}).reset_index()
            best_scores_df = scores_by_criteria_df.merge(max_scores_by_criteria_df, on=['Category', 'value'])
            best_scores_df['Best Option'] = best_scores_df.groupby(['Category'])['Option'].transform(lambda x: ', '.join(x))
            best_scores_df = best_scores_df.groupby(['Category']).agg({'Best Option': 'first'})
            st.write('Best option of each category')
            best_scores_df
            
            output_best_scores_df = scores_by_criteria_df.pivot(index=['Category'], columns=['Option'], values='value')
            ...
            for r1 in output_user_inputs.Ranks:
                ...
                for r2 in output_user_inputs.Ranks:
                    if r1==r2:
                        ...
                        rank_input = rank_sums[r2-1]*(1-new_rank_sums[scenario][r1-1])/(1-rank_sums[r1-1])
                        rows.append(rank_input)
                columns.append(rows)
